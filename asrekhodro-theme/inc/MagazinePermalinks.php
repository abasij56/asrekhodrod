...
final class MagazinePermalinks {

	public const CPT       = 'ak_magazine';
	public const BASE_PATH = 'Home/Kiosk';
	...
	public static function register_query_vars( array $vars ): array {
		$vars[] = 'ak_kiosk_route_id';

		return $vars;
	}

	public static function register_rewrites(): void {
		add_rewrite_tag( '%ak_kiosk_route_id%', '([0-9]+)' );

		add_rewrite_rule(
			'^Home/Kiosk/([0-9]+)(?:/.*)?$',
			'index.php?ak_kiosk_route_id=$matches[1]',
			'top'
		);

		if ( ! get_option( 'ak_kiosk_rewrite_id_v1' ) ) {
			flush_rewrite_rules( false );
			update_option( 'ak_kiosk_rewrite_id_v1', 1, false );
		}
	}
	...
	public static function parse_request( \WP $wp ): void {
		...
		if ( ! empty( $wp->query_vars['ak_kiosk_route_id'] ) ) {
			$route_id = (int) $wp->query_vars['ak_kiosk_route_id'];
		} else {
			$route_id = self::route_id_from_request_uri();
			if ( $route_id > 0 ) {
				$wp->query_vars['ak_kiosk_route_id'] = $route_id;
			}
		}
	...
	public static function filter_redirect_canonical( $redirect_url, string $requested_url ) {
		if ( preg_match( '#/Home/Kiosk/\d+#i', $requested_url ) ) {
			return false;
		}
	...
	public static function filter_insert_post_data( array $data, array $postarr ): array {
		...
		if ( $desired === '' ) {
			$file_id = $post_id > 0 ? (int) get_post_meta( $post_id, '_asrekhodro_file_id', true ) : 0;
			$desired = $file_id > 0 ? (string) $file_id : ( $post_id > 0 ? 'kiosk-' . $post_id : '' );
		}
	...
	public static function route_id_for_post( int $post_id ): int {
		$file_id = (int) get_post_meta( $post_id, '_asrekhodro_file_id', true );

		return $file_id > 0 ? $file_id : max( 0, $post_id );
	}
