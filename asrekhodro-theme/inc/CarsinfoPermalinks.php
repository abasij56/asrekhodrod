...
final class CarsinfoPermalinks {

	public const CPT       = 'carsinfo';
	public const BASE_PATH = 'carsinfo';
	...
	public static function register_query_vars( array $vars ): array {
		$vars[] = 'ak_carsinfo_route_id';

		return $vars;
	}

	public static function register_rewrites(): void {
		add_rewrite_tag( '%ak_carsinfo_route_id%', '([0-9]+)' );

		add_rewrite_rule(
			'^carsinfo/([0-9]+)(?:/.*)?$',
			'index.php?ak_carsinfo_route_id=$matches[1]',
			'top'
		);

		if ( ! get_option( 'ak_carsinfo_rewrite_id_v1' ) ) {
			flush_rewrite_rules( false );
			update_option( 'ak_carsinfo_rewrite_id_v1', 1, false );
		}
	}
	...
	public static function parse_request( \WP $wp ): void {
		...
		if ( ! empty( $wp->query_vars['ak_carsinfo_route_id'] ) ) {
			$route_id = (int) $wp->query_vars['ak_carsinfo_route_id'];
		} else {
			$route_id = self::route_id_from_request_uri();
			if ( $route_id > 0 ) {
				$wp->query_vars['ak_carsinfo_route_id'] = $route_id;
			}
		}
	...
	private static function route_id_from_request_uri(): int {
		...
		if ( preg_match( '#/carsinfo/(\d+)(?:/|$)#i', $path, $matches ) ) {
	...
	public static function filter_redirect_canonical( $redirect_url, string $requested_url ) {
		if ( preg_match( '#/carsinfo/\d+#i', $requested_url ) ) {
			return false;
		}
	...
	public static function filter_insert_post_data( array $data, array $postarr ): array {
		...
		if ( $desired === '' ) {
			$desired = $post_id > 0 ? 'carsinfo-' . $post_id : '';
		}
